...
messageRouter.post('/filter/paginate', async (req, res) => {
  try {
...
messageRouter.post('/filter/count', async (req, res) => {
  try {
    res.json({
...
messageRouter.put('/:messageId/accept', async (req, res) => {
  try {
...
messageRouter.put('/:messageId/decline', async (req, res) => {
  try {
    const messageDoc = await Message.findById(req.params.messageId);
    if (messageDoc == null || messageDoc.invitingGroupId == null) {
      res.status(404)
